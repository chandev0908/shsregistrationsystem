...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM abm ORDER BY lastname ASC WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM abm ORDER BY lastname,gender ASC";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printabm.php" method="POST">
         ...
         <div class="minicontainer">
            <h1> ABM DATA </h2>
             <table align="center" style="position:relative; top:30%";>
               ...
               <tr>
                ...
                <th>  <?php
                  $sqltry = "SELECT count(abmid) AS total  FROM abm WHERE Gender= 'Male'";
                  ...
                  echo '<h2>'.$numrows.'</h2>'
                   ?></th>
                   <th><?php
                   $sqltry = "SELECT count(abmid) AS total  FROM abm WHERE Gender= 'Female'";
                   $resulttry = mysqli_query($conn,$sqltry);
                   $valuestry = mysqli_fetch_assoc($resulttry);
                   $numrows = $valuestry['total'];
                   echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="adminabm.php" method="post">
          ...
          while($row = mysqli_fetch_array($search_result)):
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?abmid=<?=$row['abmid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?abmid=<?=$row['abmid']?>">DELETE</a></td>
         						</tr>
         						<?php
         					endwhile;
         					?>
