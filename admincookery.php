...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM cookery WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM cookery";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printcookery.php" method="POST">
         ...
         <div class="minicontainer">
            <h1> COOKERY DATA </h2>
             <table align="center"style="position:relative; top:30%";>
               ...
               <tr>
                  <th>  <?php
                    $sqltry = "SELECT count(cookeryid) AS total  FROM cookery WHERE Gender= 'Male'";
                    ...
                    echo '<h2>'.$numrows.'</h2>'
                     ?></th>
                     <th><?php
                     $sqltry = "SELECT count(cookeryid) AS total  FROM cookery WHERE Gender= 'Female'";
                     $resulttry = mysqli_query($conn,$sqltry);
                     $valuestry = mysqli_fetch_assoc($resulttry);
                     $numrows = $valuestry['total'];
                     echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="admincookery.php" method="post">
          ...
          while($row = mysqli_fetch_array($search_result)):
                    ?>
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?cookeryid=<?=$row['cookeryid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?cookeryid=<?=$row['cookeryid']?>">DELETE</a></td>
         						</tr>
         						<?php
         					  endwhile;
         					?>
