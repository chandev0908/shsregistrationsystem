...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM ictcss WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM ictcss";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printcss.php" method="POST">
         ...
         <div class="minicontainer">
            <h1> ICT-CSS DATA </h2>
             <table align="center"style="position:relative; top:30%";>
               ...
               <tr>
               <th>  <?php
                 $sqltry = "SELECT count(cssid) AS total  FROM ictcss WHERE Gender= 'Male'";
                 ...
                 echo '<h2>'.$numrows.'</h2>'
                  ?></th>
                  <th><?php
                  $sqltry = "SELECT count(cssid) AS total  FROM ictcss WHERE Gender= 'Female'";
                  $resulttry = mysqli_query($conn,$sqltry);
                  $valuestry = mysqli_fetch_assoc($resulttry);
                  $numrows = $valuestry['total'];
                  echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="admincss.php" method="post">
          ...
          <table align="center" style="position: relative; margin-bottom:10px;" border="1px">
         	...
         	<?php
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?cssid=<?=$row['cssid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?cssid=<?=$row['cssid']?>">DELETE</a></td>
         						</tr>
         						<?php
         			      endwhile;
         					?>
