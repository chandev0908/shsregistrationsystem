...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM dressmaking WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM dressmaking";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printdressmaking.php" method="POST">
         ...
         <div class="minicontainer">
            <h1> DRESSMAKING DATA </h2>
             <table align="center"style="position:relative; top:30%";>
               ...
               <tr>
                  <th>  <?php
                    $sqltry = "SELECT count(dressmakingid) AS total  FROM dressmaking WHERE Gender= 'Male'";
                    ...
                    echo '<h2>'.$numrows.'</h2>'
                     ?></th>
                     <th><?php
                     $sqltry = "SELECT count(dressmakingid) AS total  FROM dressmaking WHERE Gender= 'Female'";
                     $resulttry = mysqli_query($conn,$sqltry);
                     $valuestry = mysqli_fetch_assoc($resulttry);
                     $numrows = $valuestry['total'];
                     echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="admindressmaking.php" method="post">
          ...
          <table align="center" style="position: relative; margin-bottom:10px;" border="1px">
         	...
         	<?php
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?dressmakingid=<?=$row['dressmakingid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?dressmakingid=<?=$row['dressmakingid']?>">DELETE</a></td>
         						</tr>
         						<?php
         				    endwhile;
         					?>
