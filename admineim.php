...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM eim WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM eim";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printeim.php" method="POST">
         ...
         <div class="minicontainer">
            <h1> EIM DATA </h2>
             <table align="center" style="position:relative; top:30%";>
               ...
               <tr>
                  <th>  <?php
                    $sqltry = "SELECT count(eimid) AS total  FROM eim WHERE Gender= 'Male'";
                    ...
                    echo '<h2>'.$numrows.'</h2>'
                     ?></th>
                     <th><?php
                     $sqltry = "SELECT count(eimid) AS total  FROM eim WHERE Gender= 'Female'";
                     $resulttry = mysqli_query($conn,$sqltry);
                     $valuestry = mysqli_fetch_assoc($resulttry);
                     $numrows = $valuestry['total'];
                     echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="admineim.php" method="post">
          ...
          <table align="center" style="position: relative; margin-bottom:10px;" border="1px">
         	...
         	<?php
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?eimid=<?=$row['eimid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?eimid=<?=$row['eimid']?>">DELETE</a></td>
         						</tr>
         						<?php
         					    endwhile;
         					?>
