...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM stem WHERE CONCAT (lrn, firstname, lastname, middlename) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $queryqew = "SELECT * FROM stem";
...
<body>
  <div class="page-wrap">
       ...
       <form action="./printmanage/printstem.php" method="POST">
       ...
       <section id="showcase">
         <div class="minicontainer">
            <h1> STEM DATA </h2>
             <table align="center" style="position:relative; top:30%";>
                 ...
                 echo '<h2>'.$row.'</h2>'
                  ?><tr>
                  <th>  <?php
                    $sqltry = "SELECT count(stemid) AS total  FROM stem WHERE Gender= 'Male'";
                    ...
                    echo '<h2>'.$numrows.'</h2>'
                     ?></th>
                     <th><?php
                     $sqltry = "SELECT count(stemid) AS total  FROM stem WHERE Gender= 'Female'";
                     $resulttry = mysqli_query($conn,$sqltry);
                     $valuestry = mysqli_fetch_assoc($resulttry);
                     $numrows = $valuestry['total'];
                     echo 'FEMALE COUNTS :';
         	...
         	?>
          <form action="adminstem.php" method="post">
          ...
          <table align="center" style="position: relative;  margin-bottom:10px;" border="1px">
         	...
         	<?php
          $query="SELECT stemid FROM stem ORDER BY gender ASC";
          $query_run= mysqli_query($conn,$query);
          $row = mysqli_num_rows($query_run);
                       ...
                       <td><?=$row['lrn'];?></td>
         							...
         							<td><?=$row['section'];?></td>
         							<td><?=$row['strand'];?></td>
         							<td><a href="update.php?stemid=<?=$row['stemid']?>">UPDATE</a></td>
         							<td><a href="editconn.php?stemid=<?=$row['stemid']?>">DELETE</a></td>
         						</tr>
         						<?php
         			      endwhile;
         					?>
